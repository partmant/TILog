...
const AdminPage = () => {
    ...
    const loadData = async () => {
        setIsLoading(true);
        try {
            const [memberRes, reportRes, statsRes] = await Promise.all([fetchMemberList(0, 10), fetchRecentReports(), fetchReportStats()]);
    ...
    const formatReportData = (report) => {
        // 사유(ReasonType) 매핑 객체
        const reasonMap = {
            'ABUSE': {category: '욕설/비방', color: '#fee2e2', textColor: '#ef4444'},
            'AD': {category: '광고/홍보', color: '#ffedd5', textColor: '#f97316'},
            'ADULT': {category: '음란물', color: '#fce7f3', textColor: '#db2777'},
            'PERSONAL_INFO': {category: '개인정보', color: '#d1fae5', textColor: '#10b981'},
            'SPAM': {category: '도배/스팸', color: '#dbeafe', textColor: '#3b82f6'},
            'ETC': {category: '기타', color: '#f3f4f6', textColor: '#6b7280'}
        };

        const info = reasonMap[report.reasonType] || reasonMap['ETC'];
        ...
        return {
            ...
            status: report.status === 'PROCESSED' ? '완료' : (report.status === 'IN_PROGRESS' ? '검토중' : '대기'),
            statusColor: report.status === 'PROCESSED' ? '#10b981' : '#ef4444',
            rawStatus: report.status
        };
    };

    // 🔥 전체 TIL 카드 삭제
    const statsData = [{title: '전체 회원', count: `${members?.totalElements || 0}명`, icon: '👥'}, {
    ...
    return (<div className="admin-page">
        ...
        <div className="admin-stats-row">
            {statsData.map((stat, index) => (<div className="admin-stat-card" key={index}>
            ...
            <div className="admin-panel">
                ...
                <ul className="admin-report-list">
                    {recentReports.length === 0 && !isLoading ? (
                        <li className="admin-report-item">접수된 신고가 없습니다.</li>) : (recentReports.map((report) => {
    ...
    <strong style={{color: '#4b5563', marginRight: '8px'}}>
        [{report.reporterNickname || '알수없음'} ➡️ {report.reportedNickname || '알수없음'}]
                                            ...
                                            <span style={{color: '#374151'}}>{formatted.detail}</span>
</span>
                        ...
                        >
                            ...
                            <div className="admin-op-info">
                                <span className="op-title"
                                      style={{fontSize: '15px', color: '#111827'}}>회원 및 권한 통합 관리</span>
        ...
        {/* 회원/권한 모달 */}
        {isModalOpen && (<div className="admin-modal-overlay">
            <div className="admin-modal-card" style={{maxWidth: '800px'}}>
                <h3>회원 및 권한 통합 관리</h3>

                <div className="admin-modal-form" style={{
                    display: 'flex', flexDirection: 'row', gap: '16px', alignItems: 'flex-end', marginBottom: '24px'
                }}>
                    <div style={{flex: 2}}>
                        ...
                        <select value={targetMemberId} onChange={(e) => setTargetMemberId(e.target.value)}
                            ...
                            {members?.content?.map(member => (<option key={member.memberId} value={member.memberId}>
                ...
                <div style={{
                    maxHeight: '300px', overflowY: 'auto', border: '1px solid #e5e7eb', borderRadius: '12px'
                }}>
                    <table style={{
                        width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px'
                    }}>
                        ...
                        <tbody>
                        {members?.content?.map((m) => (<tr key={m.memberId} style={{borderBottom: '1px solid #f3f4f6'}}>
        ...
        {/* 신고 처리 모달 */}
        {isReportModalOpen && (<div className="admin-modal-overlay">
